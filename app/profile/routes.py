...
# @login_required
def create_profile(): 
    return render_template('profile.html', user_name=current_user.get_full_name(), profile_title=current_user.profile_title)
...
@profile.route('/api/profile/title', methods=['PUT'])
@login_required
def update_profile_title():
    try:
        data = request.json
        ...
        current_user.profile_title = new_title
        db.session.commit()
        
        return jsonify({
            'message': 'Profile title updated successfully',
            'title': new_title
        })
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': 'Could not update profile title'}), 500
